...
class OssClient
{
    ...
    public function signUrl($bucket, $object, $timeout = 60, $method = self::OSS_HTTP_GET, $options = NULL)
    {
        $this->precheckCommon($bucket, $object, $options);
        //method
        if (self::OSS_HTTP_GET !== $method && self::OSS_HTTP_PUT !== $method && self::OSS_HTTP_HEAD !== $method ) {
        ...
        return $this->clientauth($options);
    }
    ...
    /*******生成get签名************/
    // public  function  headgetauth(){
    //    $signauth=array();
    //    $time=gmdate("D, d M Y H:i:s")." GMT";
    //    $szStr = "GET\n\n\n".$time."\n"."/mxtd-fur-pak/2.txt";
    //    $signature = base64_encode(hash_hmac('sha1', $szStr, $this->accessKeySecret,ture));  
    //    $signauth['getdate']=$time;
    //    $signauth['getsign']=$signature;
    //    return  $signauth;
    // }
     ...
     * @throws RequestCore_Exception
     */
   private function clientauth($options)
    {
        OssUtil::validateOptions($options);
        //验证bucket，list_bucket时不需要验证
        $this->authPrecheckBucket($options);
        ...
        $request->add_header('Authorization', 'OSS ' . $this->accessKeyId . ':' . $signature);  
        //HEAD
        $pathinfo=parse_url($request->request_url); 
        $filename=substr($pathinfo['path'],1);   
        if($request->method==="HEAD"){
            $time=gmdate("D, d M Y H:i:s")." GMT"; 
            $szStr = "HEAD\n\n\n".$time."\n"."/".$bucket."/".$filename;
            $signature = base64_encode(hash_hmac('sha1', $szStr, $this->accessKeySecret,ture));  
            $headsignauth="OSS ".C('accesskeyid').":".$signature;
            // $signed_url= $this->requestUrl . $conjunction . self::OSS_URL_ACCESS_KEY_ID . '=' . rawurlencode($this->accessKeyId) . '&' . self::OSS_URL_EXPIRES . '=' . $options[self::OSS_PREAUTH] . '&' . self::OSS_URL_SIGNATURE . '=' . $signature; 
            // $signed_url1 = $this->requestUrl . $conjunction . self::OSS_URL_ACCESS_KEY_ID . '=' . rawurlencode($this->accessKeyId) . '&' . self::OSS_URL_EXPIRES . '=' . $options[self::OSS_PREAUTH] . '&' . self::OSS_URL_SIGNATURE . '=' . rawurlencode($signature);
            // dump($signed_url1);exit;
               return $headsignauth;
        }
        //GET
       ...
       $signature = base64_encode(hash_hmac('sha1', $szStr, $this->accessKeySecret,ture));  
       $signauth['getdate']=$time;
       $signauth['getsign']="OSS ".C('accesskeyid').":".$signature;
       $signauth['geturl']=$this->requestUrl;
       $signauth['accesskeyid']=C('accesskeyid'); 
       return  $signauth;
      }
         //PUT
       ...
       $szStr = "PUT\n\n\n".$time."\n"."/".$bucket."/".$filename;
       $signature = base64_encode(hash_hmac('sha1', $szStr, $this->accessKeySecret,ture));  
       $signauth['putdate']=$time;
       $signauth['putsign']="OSS ".C('accesskeyid').":".$signature;
       $signauth['putturl']=$this->requestUrl;
       $signauth['accesskeyid']=C('accesskeyid');
       return  $signauth;
      }
