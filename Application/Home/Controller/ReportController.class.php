...
class ReportController extends Controller {
	...
	public function lljz2(){
		$userid=1;
		
		$list=M('user_code')->alias('a')
		->join('left join jj_code as b on b.code=a.code')
		->join('left join jj_guess as c on c.code=a.code')->
		where("a.user_id=$userid and selected=1")->select();
//		echo M('user_code')->_sql();
//		dump($list);
...
//		$list=$t->getMoniList(8,$t->code,"2016-09-11 12:00:00","2017-09-11 12:00:00");
//		dump($list);
		$syList=$t->updateSellFen($list);
		
		
		$t->updateUse2($userid,$code);
		$syList=$t->getZhouqi($userid,$code,'2017-01-25','2017-09-26'); 
		$this->assign('syList',$syList['syList']);
		$this->assign('syList2',$syList['syList2']);
		$this->assign('money',$syList['money']);
		$this->assign('shouyi',$syList['shouyi']);
		$this->assign('money1',$syList['money1']);
		$this->assign('shouyi1',$syList['shouyi1']);
		$this->assign('money2',$syList['money2']);
		$this->assign('shouyi2',$syList['shouyi2']); 
		$this->assign('chiyje',$syList['chiyje']); 
		$sy=round(($syList['shouyi']/$syList['money'])*100,2);
		$this->assign('syl',$sy.'%');
		$this->display();
	}
function quickSort($arr,$name) {
    ...
    //选择第一个元素作为基准
    $base_num = $arr[0][$name];
    $base= $arr[0];//dump($base);
    //遍历除了标尺外的所有元素，按照大小关系放入两个数组内
    //初始化两个数组
    $left_array = array();  //小于基准的
    $right_array = array();  //大于基准的
    for($i=1; $i<$length; $i++) {
        if($base_num > $arr[$i][$name]) {
            //放入左边数组
            $left_array[] = $arr[$i];
        } else {
            //放入右边
    ...
    $left_array = $this->quickSort($left_array,$name);
    $right_array = $this->quickSort($right_array,$name);
    //合并
